...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Import Kaikki/Wiktionary English translations into SQLite."
    )
    parser.add_argument(
        "--source",
        default="data/models/wiktionary-en/raw-wiktextract-data.jsonl.gz",
        help="Path to downloaded Kaikki English JSONL or JSONL.GZ dump.",
    )
    parser.add_argument(
        "--output",
        required=True,
    ...
    parser.add_argument(
        "--target-lang",
        required=True,
        help="Wiktextract translation code to keep, e.g. ru or uk.",
    )
    parser.add_argument(
        "--limit",
        type=int,
...
def entry_type_for_lemma(lemma: str) -> str:
    ...
    return "phrase" if len(words) >= 2 else "word"
            ...
            for line in handle:
                ...
                for sense_index, sense in enumerate(senses):
                    ...
                    for trans in sense.get("translations") or []:
                        if str(trans.get("code") or "") != target_lang:
                            continue
                        translated = normalize_translation(
                            str(trans.get("word") or "")
                        )
                        if translated:
                            sense_translations.append(translated)
                    for priority, translated in enumerate(sense_translations):
                        translations.append((translated, sense_index, priority))
                top_level_translations = []
                for trans in item.get("translations") or []:
                    if str(trans.get("code") or trans.get("lang_code") or "") != target_lang:
                        continue
                    translated = normalize_translation(str(trans.get("word") or ""))
                    if translated:
                        top_level_translations.append(translated)
                for priority, translated in enumerate(top_level_translations):
                    translations.append((translated, 0, priority))
                if not translations:
                    continue
                ...
                if form_rows:
                    conn.executemany(
                        "INSERT INTO forms(form, lemma, pos) VALUES (?, ?, ?)",
                        form_rows,
                    )

                conn.executemany(
                    """
                    INSERT INTO translations(entry_id, translation, sense_index, priority)
                    VALUES (?, ?, ?, ?)
                    """,
                    [
                        (entry_id, translation, sense_index, priority)
                        for translation, sense_index, priority in translations
                    ],
                )
                ...
                if definitions:
                    conn.executemany(
                        """
                        INSERT INTO definitions(entry_id, definition, sense_index)
                        VALUES (?, ?, ?)
                        """,
                        [
                            (entry_id, definition, sense_index)
                            for definition, sense_index in definitions
                        ],
                    )
...
def main() -> None:
    ...
    entries, translations = import_dump(
        source=source,
        output=output,
