...
description: Create and audit the complete UK dictionary files for one LEXO book from parallel EN/UK segments. Build verified word seeds, minimal reusable semantic blocks, the UK book layer, and occurrence-level verification; reject compositional or oversized groups and prove every occurrence. Use for new UK book dictionaries, missing/excessive block audits, UK book dictionary rebuilds, or `$lexo-uk-book-audit` for UK books.
---

# LEXO UK Book Audit

Work on one requested UK book at a time. Treat current code, `Docs/History/INDEX.md`, and the book's `book_layer_uk.json` as sources of truth. Read `references/block-schema.md` and `references/uk-book-layer-schema.md` before analyzing or writing book files.
...
2. Find the requested `book_layer_uk.json` under `Studio/Backend/data/dictionaries/library_uk/books/`.
...
6. Write one concise Ukrainian `explanation` describing what the construction means or does. Make it reusable across books; do not explain the surrounding sentence.
   Keep `type` as the schema's technical enum, but never use that English enum as user-facing text. The application must render its approved Ukrainian label: `phrasal_verb → фразове дієслово`, `fixed_expression → стійкий вираз`, `collocation → словосполучення`, `grammar_construction → граматична конструкція`, `prepositional_group → прийменникова група`, `name_group → група власної назви`, `reordered_block → блок зі зміненим порядком слів`. Do not invent a new English display label.
...
Keep the reusable semantic description only in `explanation`. Example: in `for the rest of the day → до кінця дня`, the minimal block `the rest of` owns `кінця`; `for` independently owns `до`, and `day` owns `дня`.
For that block, keep `dictionary_translation: "решта чогось"` and occurrence `translation: "кінця"`; they are complementary values, not alternatives to overwrite.
...
- Reject a grammatical remnant as a word translation. For example,
  `wrong → тому` is invalid because `тому` alone does not carry the meaning of
  `wrong`; an honest reusable meaning is `не той` or `неправильний`.
...
- Treat the group as alignment evidence, never as a phrase block or dictionary
  meaning. Keep it book-local in `word_to_word_uk.json`; never copy it to block
...
- Make the group own every target token required by its natural realization,
  including structural additions with no separate English token. Do not classify
  those owned tokens as exceptions, ignored words, or uncovered target residue.
...
After word and block ownership is stable, tokenize every Ukrainian `parallel[].translation`
and subtract all independently owned word spans. Classify every remaining target span
semantically from the complete EN/UK segment as exactly one of:
...
Write the decisions to `word_to_word_uk.json.target_coverage[]` with exact target
indexes, all participating `source_word_ids[]`, and a concise Ukrainian `reason`.
...
`word_to_word_uk.json.alignment_groups[]` using the schema in
`references/uk-book-layer-schema.md`. A group may cover target tokens already
...
The second command also requires and validates the sibling UK seeds and
skill-owned verification `word_to_word_uk.json`. Treat any validator error
...
2. Update `book_layer_uk.json` atomically.
3. Build a skill-owned occurrence-level `word_to_word_uk.json` directly
...
5. Require complete target-side coverage. Preserve exact word translation spans and
   ...
   to absorb a neighboring Ukrainian token.
5. Run `scripts/validate_block_layer.py`, `scripts/validate_uk_book_layer.py`,
