...
class CardsListScreen extends StatefulWidget {
  ...
  final Future<SavedCardItem> Function(String cardId, String direction)?
      applyReviewResult;
...
class _CardsListScreenState extends State<CardsListScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final payload =
          loader != null ? await loader() : await widget.api!.getSavedCards();
      if (!mounted) {
        return;
      }
  ...
  Future<void> _startReview() async {
    try {
      ...
      final payload = reviewLoader != null
          ? await reviewLoader()
          : await widget.api!.getReviewCards();
      if (!mounted) {
        return;
      }
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Could not start review: $error')),
      );
    }
  }
  ...
  Future<void> _deleteCard(SavedCardItem item) async {
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Deleted: ${item.headText}')),
      );
      await _load();
    } catch (error) {
      if (!mounted) {
  ...
  Future<void> _playWordAudio(SavedCardItem item) async {
    ...
    if (resolver != null) {
      ...
      final audioFile =
          File('${tempDir.path}/lexo_card_word_${item.headText.hashCode}.wav');
              ...
              child: _busy
                  ...
                  : _error != null
                      ...
                      : items.isEmpty
                          ? const Center(
                              child: Text(
                                  'No cards yet. Add them from Reader with a long press.'),
                            )
                          : RefreshIndicator(
                              onRefresh: _load,
