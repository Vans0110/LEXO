...
class TtsPanel extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final selectedLevelId =
        selectedLevelIds.isEmpty ? null : selectedLevelIds.first;
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
                ...
                items: levels
                    .map(
                      (level) => DropdownMenuItem<int>(
                        value: level.id,
                        child: Text(
                            '${level.name} ${(level.playbackSpeed).toStringAsFixed(2)}x'),
                      ),
                    )
                    .toList(),
                onChanged: busy
              ...
              FilledButton(
                onPressed:
                    busy || selectedVoiceId == null || selectedLevelId == null
                        ? null
                        : onGenerate,
              ...
              OutlinedButton(
                onPressed:
                    busy || selectedJob == null ? null : onOverwriteGenerate,
                child: const Text('Overwrite'),
              ),
              const SizedBox(height: 12),
              if (selectedJob == null)
                const Text('No generation for this voice yet')
              else ...[
                Text(
                ...
                const SizedBox(height: 6),
                Text(
                    '${selectedJob.readySegments}/${selectedJob.totalSegments}'),
              ],
