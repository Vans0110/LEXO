...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> _loadStatus() async {
    ...
    try {
      ...
      setState(
          () => _state = _state.copyWith(busy: false, error: error.toString()));
    } finally {
  ...
  Future<void> _pickAndImport() async {
    final importStartMessage = 'HOME_IMPORT_PICK_START busy=${_state.busy} '
        'items=${_state.library?.items.length ?? -1} openingBookId=${_state.openingBookId ?? ''}';
    ...
    try {
      ...
      final importFileMessage =
          'HOME_IMPORT_FILE name=${pickedFile.name} path=${pickedFile.path}';
  ...
  Future<bool> _confirmDelete(LibraryBookItem item) async {
    final result = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text(
            'Delete "${item.title}" from the library? This will remove text and TTS files.'),
        actions: [
          TextButton(
            ...
            child: Column(
              ...
              children: [
                ...
                FilledButton(
                  ...
                  child: Text(_state.busy ? 'Please wait...' : 'Load TXT'),
                ),
                const SizedBox(height: 16),
                Expanded(
                  child: library == null || library.items.isEmpty
                      ? const Center(child: Text('The library is empty'))
                      : ListView.separated(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            return Card(
                              child: ListTile(
                                ...
                                subtitle: Text(
                                  '${item.sourceLang} -> ${item.targetLang} | ${item.modelName} | position ${item.currentParagraphIndex + 1}',
                                ),
                                trailing: Row(
                                          ...
                                          : () async {
                                              ...
                                              try {
                                                await _controller
                                                    .openBook(item.id);
                                                _uiTrace(
                                                    'HOME_OPEN_API_OK bookId=${item.id}');
                                                if (!mounted) return;
                                                setState(
                                                  () => _state =
                                                      _state.copyWith(
                                                ...
                                                if (widget.onBookOpened !=
                                                    null) {
                                                  await _loadStatus();
                                                  return;
                                                }
                                                _uiTrace(
                                                    'HOME_OPEN_READER_PUSH bookId=${item.id}');
                                                ...
                                                _uiTrace(
                                                    'HOME_OPEN_ERROR bookId=${item.id} error=$error');
                                                if (!mounted) {
                                                  return;
                                                }
                                                setState(
                                    ...
                                    OutlinedButton(
                                      onPressed: _state.busy
                                          ...
                                          : () async {
                                              ...
                                              developer.log(deleteStartMessage,
                                                  name: 'LEXO_DELETE');
                                              ...
                                              try {
                                                ...
                                                final deleteOkMessage =
                                                    ...
                                                    'activeBookId=${nextState.library?.activeBookId ?? ''}';
                                                developer.log(deleteOkMessage,
                                                    name: 'LEXO_DELETE');
                                                debugPrint(deleteOkMessage);
                                                setState(
                                                    () => _state = nextState);
                                                if (nextState.library != null) {
                                                  widget.onLibraryLoaded?.call(
                                                      nextState.library!);
                                                }
                                              } catch (error) {
