    ...
    def do_GET(self) -> None:
        ...
        try:
            ...
            if path == "/tts/levels":
                ...
                return
            if path == "/google-translate/usage":
                self._send_json(
                    HTTPStatus.OK,
                    STORAGE.google_translate_usage_summary(
                        _query_value(query, "month"),
                    ),
                )
                return
            if path == "/tts/state":
    ...
    def do_POST(self) -> None:
        ...
        try:
            ...
            if path == "/books/delete":
                ...
                self._send_json(HTTPStatus.OK, result)
                return
            if path == "/books/clean-artifacts":
                result = STORAGE.clean_book_artifacts(
                    payload.get("book_id"),
                    voice_ids=payload.get("voice_ids") or [],
                )
                self._send_json(HTTPStatus.OK, result)
                return
            ...
            if path == "/books/rebuild-dictionary-manifest":
                ...
                self._send_json(HTTPStatus.OK, result)
                return
            if path == "/books/rebuild-library-dictionary":
                result = STORAGE.rebuild_book_library_dictionary(
                    payload.get("book_id"),
                    target_lang=payload.get("target_lang", "ru"),
                )
                self._send_json(HTTPStatus.OK, result)
