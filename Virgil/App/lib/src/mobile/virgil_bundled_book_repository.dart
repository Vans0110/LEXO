...
class VirgilBundledBookRepository {
  static const _productionCloudBaseUrl =
      'https://pub-05fee08c1a5741d6a9a57d087c327c96.r2.dev/virgil/library';
  static const _cloudBaseUrl = String.fromEnvironment(
    'VIRGIL_LIBRARY_BASE_URL',
    defaultValue: _productionCloudBaseUrl,
  );
  ...
  Future<List<VirgilBundledBookInfo>> _listCloudBooks({
    ...
    final payload = await _loadCloudIndex(
      baseUrl,
      forceRefresh: forceRefresh,
    );
  ...
  Future<String> importBook(
    ...
    try {
      ...
      await verifyVirgilBookArchiveHash(zipFile, info.contentHash);
  ...
  Future<Map<String, dynamic>> _loadCloudIndex(
    String baseUrl, {
    bool forceRefresh = false,
  }) async {
    final cached = _cloudIndexCache;
    final cachedAt = _cloudIndexCachedAt;
  ...
  Future<Map<String, dynamic>> _loadCloudIndexFromNetwork(
    ...
    for (var attempt = 0; attempt < 3; attempt++) {
      try {
        ...
        _cloudIndexCachedAt = DateTime.now();
        return payload;
      } catch (error) {
        lastError = error;
        if (attempt < 2) {
...
Future<void> verifyVirgilBookArchiveHash(
  File zipFile,
  String expectedHash,
) async {
  ...
  if (!RegExp(r'^[0-9a-f]{64}$').hasMatch(normalizedExpected)) {
    throw const FormatException(
      'Cloud book package does not contain a valid SHA-256 hash.',
    );
  }
  final actualHash = (await sha256.bind(zipFile.openRead()).first).toString();
  if (actualHash != normalizedExpected) {
    throw const FormatException(
      'Downloaded book package failed SHA-256 verification.',
    );
  }
}
