...
class LexoApiClient {
  ...
  Future<DetailSheetPayload> getDetailSheet({
    ...
    final data =
        await _get('/reader/detail-sheet?book_id=$bookId&word_id=$wordId');
  ...
  Future<List<int>> downloadWordAudio(String word, {String? voiceId}) async {
    ...
    final uri = Uri.parse(
        '$baseUrl/word/audio?word=${Uri.encodeQueryComponent(word)}$voiceQuery');
    Object? lastError;
    for (var attempt = 1; attempt <= _audioDownloadMaxAttempts; attempt += 1) {
      ...
      try {
        ...
        if (response.statusCode >= 400) {
          throw LexoApiException(
              'Word audio download failed with status ${response.statusCode}');
        }
        return bytes;
  ...
  Future<Map<String, dynamic>> downloadMobileBookPackageChunked(
    ...
    final package = <String, dynamic>{
      ...
      'dictionary_manifests': <String, dynamic>{},
    ...
    for (final part in partItems) {
      ...
      switch (kind) {
        ...
        case 'reader_meta':
          readerPayload
            ...
            ..['current_paragraph_index'] =
                payload['current_paragraph_index'] ?? 0;
          break;
        ...
        case 'dictionary_manifest':
          ...
          break;
        case 'dictionary_manifests':
          package['dictionary_manifests'] = payload;
          break;
  ...
  Future<TtsPackageState> getTtsPackageState({
    ...
    final data =
        await _get('/tts/package-state?book_id=$bookId&voice_id=$voiceId');
  ...
  Future<List<int>> downloadTtsAudio({
    ...
    throw lastError is Exception
        ? lastError
        : LexoApiException(
            'Audio download failed after $_audioDownloadMaxAttempts attempts');
  }
  ...
  Future<void> saveReaderPosition(String bookId, int paragraphIndex) async {
    await _post('/reader/position',
        {'book_id': bookId, 'paragraph_index': paragraphIndex});
  }
  ...
  Future<Map<String, dynamic>> _get(String path, {Duration? timeout}) async {
    ...
    final request =
        await client.getUrl(uri).timeout(timeout ?? const Duration(days: 1));
    request.persistentConnection = false;
    request.headers.set(HttpHeaders.connectionHeader, 'close');
    final response =
        await request.close().timeout(timeout ?? const Duration(days: 1));
    try {
  ...
  Future<Map<String, dynamic>> _post(String path, Map<String, dynamic> payload,
    ...
    final request =
        await client.postUrl(uri).timeout(timeout ?? const Duration(days: 1));
    request.persistentConnection = false;
    request.headers.set(HttpHeaders.connectionHeader, 'close');
