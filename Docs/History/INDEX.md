...
- Экран книг главы получил центрированную адаптивную сетку из двух или трёх
  колонок без изменения размера карточек.
...
- Sync to R2 теперь удаляет публикации, для которых больше нет TXT в Workbench:
  очищаются локальные ZIP/обложки, перестраивается индекс и удаляются лишние
  файлы на R2; `New Student` и `Late for Class` удалены из опубликованной библиотеки.
